...
#                                                     #
# Usage: Script to build arm-linux-gnueabi(arm32)     #
#                                                     #
#######################################################
...
#download tarballs here
	...
	wget ${DLFLAGS} -O ${_TOP_DIR_}/package/glibc-${_LIB_VER_}.tar.xz    ${SW_DL_URL}/gnu/glibc/glibc-${_LIB_VER_}.tar.xz
...
ifeq ($(call FILE_DIR_EXIST,glibc-${_LIB_VER_},${PRJROOT}/src),)
	tar -Jxvf ${_TOP_DIR_}/package/glibc-${_LIB_VER_}.tar.xz    -C ${PRJROOT}/src
endif
	@echo "unzip src end"
...
ifeq ($(call FILE_DIR_EXIST,Makefile,${_PRJROOT_INN_}/bld/${GCC_VERSION}-pass2),)
	${_PRJROOT_INN_}/src/${GCC_VERSION}/configure --target=${TARGET} --prefix=${_PREFIX_INN_} --enable-languages=c,c++ --enable-shared ${_OPTIONS_} 2>&1 |tee configure.out
endif
	make -w all install 2>&1 |tee make.out
